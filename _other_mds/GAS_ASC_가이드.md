...
void URAC_AttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
    ...
    {
        OnHealthChanged.Broadcast(OldValue, NewValue);  // UI에서 바인딩 가능
    }
}
```
...
**위치:** `Source/MyRatchet/Public/Frameworks/RAC_PlayerState.h`

**역할:**
- ASC와 AttributeSet을 소유 및 관리
...
class UAbilitySystemComponent* AbilitySystemComponent;  // ASC

UPROPERTY()
class URAC_AttributeSet* AttributeSet;  // AttributeSet
```

**사용 방법:**

```cpp
// 캐릭터에서 PlayerState 접근
ARAC_PlayerState* PS = GetPlayerState<ARAC_PlayerState>();

// PlayerState에서 ASC 접근
...
### 3. ARAC_CPP_Character

**위치:** `Source/MyRatchet/Public/Characters/RAC_CPP_Character.h`

**추가된 기능:**

```cpp
// IAbilitySystemInterface 구현
class ARAC_CPP_Character : public ACharacter, public IAbilitySystemInterface
...
```cpp
// 직접 속성값 변경 (선택적)
URAC_AttributeSet* Attributes = ...
Attributes->SetHealth(Attributes->GetHealth() - DamageAmount);
```
...
#### 새로운 속성 추가 시
- [ ] RAC_AttributeSet.h에 FGameplayAttributeData 선언
- [ ] ATTRIBUTE_ACCESSORS 매크로 적용
- [ ] RAC_AttributeSet.cpp 생성자에 초기값 설정
- [ ] (선택) PostGameplayEffectExecute에 콜백 추가
- [ ] (선택) 델리게이트 선언
